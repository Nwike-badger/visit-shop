...
const AddressForm = ({ address, setAddress, loading }) => {
  ...
  const handlePhoneChange = (e) => {
    let val = e.target.value.replace(/\D/g, ''); // strip everything non-numeric

    // If pasted with country code e.g. "2348067087765" or "+2348067087765"
    if (val.startsWith('234') && val.length > 10) {
      val = val.slice(3);
    }

    // Strip leading zero — the +234 prefix is already shown in the UI
    // e.g. "08067087765" → "8067087765"
    if (val.startsWith('0')) {
      val = val.slice(1);
    }

    // Cap at 10 digits AFTER the strip so we never lose real digits
    if (val.length > 10) {
      val = val.slice(0, 10);
    }
      ...
      <div>
        ...
        <div className="relative flex items-center">
          ...
          <input
            ...
            pattern="[0-9]{10}"
            title="Please enter exactly 10 digits after the country code (e.g. 8067087765)"
        ...
        {address.phoneNumber && address.phoneNumber.length > 0 && address.phoneNumber.length < 10 && (
          <p className="text-[10px] text-red-500 font-bold mt-1.5">
            {10 - address.phoneNumber.length} more digit{10 - address.phoneNumber.length !== 1 ? 's' : ''} needed.
          </p>
        )}
        {address.phoneNumber && address.phoneNumber.length === 10 && (
          <p className="text-[10px] text-green-600 font-bold mt-1.5">✓ Looks good!</p>
        )}
      </div>
