...
const SignupPage = () => {
  ...
  const resend = async () => {
    setResending(true);
    try {
      await api.post('/v1/auth/resend-verification', { email: formData.email });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4 py-12 font-sans">
        <div className="max-w-md w-full bg-white p-8 rounded-2xl shadow-sm border border-gray-100 text-center">
          ...
          <p className="text-sm font-bold text-gray-900 mb-6">{formData.email}</p>
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4 py-12 font-sans">
      <div className="max-w-md w-full bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div className="mb-6 flex justify-center">
          <GoogleLogin onSuccess={handleGoogleSuccess} onError={() => toast.error('Google signup popup closed or failed')} useOneTap shape="rectangular" size="large" text="signup_with" width="100%" />
        </div>
        ...
        <form onSubmit={handleSignup} className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              <input type={showPassword ? "text" : "password"} name="password" required minLength={6} className="w-full p-3.5 pr-12 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-blue-500 outline-none transition-all text-sm font-medium" onChange={handleChange} disabled={loading} placeholder="••••••••" />
              <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 p-2 text-gray-400 hover:text-gray-600 transition-colors focus:outline-none" onClick={() => setShowPassword(!showPassword)} aria-label={showPassword ? "Hide password" : "Show password"}>
                {showPassword ? (
          ...
          <button disabled={loading} className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold uppercase tracking-widest text-xs mt-2 hover:bg-blue-700 transition-all shadow-lg shadow-blue-100 hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        <div className="mt-8 text-center text-sm font-medium text-gray-500">
          ...
          <Link to="/login" state={{ from: location.state?.from }} className="font-bold text-blue-600 hover:text-blue-700 transition-colors">Log In</Link>
        </div>
      </div>
    </div>
  );
