...
import { useAuth } from '../../context/AuthContext'; // 🔥 1. IMPORT GLOBAL CONTEXT

const AccountPage = () => {
  const navigate = useNavigate();
  const { updateUser } = useAuth(); // 🔥 2. PULL IN UPDATE FUNCTION
  ...
  const handleAddressUpdate = async (e) => {
    ...
    try {
        ...
        setUser(res.data); // Updates local page
        
        // 🔥 3. THE FIX: SYNC WITH GLOBAL STATE SO NAVBAR & CHECKOUT INSTANTLY UPDATE!
        if (updateUser) {
            updateUser(res.data); 
        }
  ...
  // 🛡️ Smart Validation: Make sure it actually has the new fields before calling it 100% complete
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 space-y-8">
            ...
            <div className="bg-white p-6 sm:p-8 border border-gray-100 shadow-sm rounded-2xl flex items-center gap-6">
                <div className="w-20 h-20 bg-gray-900 rounded-full flex items-center justify-center text-white text-3xl font-black shadow-lg">
            ...
            <div className="bg-white p-6 sm:p-8 border border-gray-100 shadow-sm rounded-2xl">
                ...
                {isEditingAddress ? (
                    <form onSubmit={handleAddressUpdate} className="max-w-xl">
                        
                        <AddressForm address={addressForm} setAddress={setAddressForm} loading={saving} />

                        <div className="flex gap-3 pt-6">
                            <button type="submit" disabled={saving} className="bg-gray-900 text-white font-bold py-3.5 px-8 rounded-xl shadow-lg hover:bg-black transition disabled:opacity-50">
                                {saving ? 'Saving...' : 'Save Address'}
                            </button>
                            {isAddressComplete && (
                                <button type="button" onClick={() => setIsEditingAddress(false)} className="text-gray-500 font-bold px-4 hover:text-gray-800 transition-colors bg-gray-50 rounded-xl">Cancel</button>
                            )}
                        </div>
                    </form>
                ) : isAddressComplete ? (
                    <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                        ...
                        <p className="text-gray-500 text-sm mb-3">{user.defaultAddress.phoneNumber}</p>
                        <p className="text-gray-700">{user.defaultAddress.streetAddress}</p>
                        <p className="text-gray-700">{user.defaultAddress.city}, {user.defaultAddress.state}</p>
                        <p className="text-gray-500 mt-1">{user.defaultAddress.country}</p>
                    </div>
                ) : (
                    <div className="text-center py-10 bg-gray-50 border border-dashed border-gray-300 rounded-xl">
                        <p className="text-gray-500 mb-4">Your address information is missing or incomplete.</p>
